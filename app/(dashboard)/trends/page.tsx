...
export default function TrendsPage() {
  const { habits, habitEntries } = useApp()

  const medications = habits.filter((h) => h.category === 'medication')

  // Build last-7-days chart data from habitEntries for medication habits
  const days = Array.from({ length: 7 }, (_, i) => {
    const date = subDays(new Date(), 6 - i)
    const dateStr = format(date, 'yyyy-MM-dd')
    const count = habitEntries.filter(
      (e) => medications.some((m) => m.id === e.habitId) && e.date === dateStr && e.status === 'success'
    ).length
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-8">
      ...
      >
        ...
        <motion.div variants={staggerItem} className="bg-surface-container-lowest rounded-lg p-6 shadow-card">
          <p className="text-xs font-bold uppercase tracking-widest text-on-surface-variant/60 mb-4">
            Medication Logged — Last 7 Days
          </p>
          {medications.length === 0 ? (
            <p className="text-sm text-on-surface-variant py-8 text-center">
              No medications configured yet. Add one in the Log tab.
            </p>
          ) : (
        ...
        <motion.div variants={staggerItem} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={card.label} className="bg-surface-container-lowest rounded-lg p-5 shadow-card flex flex-col gap-3">
              <div className="p-2.5 bg-secondary/10 rounded w-fit">
